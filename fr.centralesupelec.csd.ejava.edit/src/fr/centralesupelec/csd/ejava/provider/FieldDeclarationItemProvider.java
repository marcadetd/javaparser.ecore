...
public class FieldDeclarationItemProvider extends BodyDeclarationItemProvider {
    ...
    public String getText( Object object ) {
        ...
        if( fieldDeclaration.getVariables().size() == 0 ) {
            return getString( "_UI_FieldDeclaration_type" );
        }
        StringBuilder result = new StringBuilder();
        boolean first = true;
        for( VariableDeclarator v : fieldDeclaration.getVariables() ) {
            if( !first ) result.append( ", " );
            IItemLabelProvider labelProvider = ( IItemLabelProvider ) getAdapterFactory().adapt( v,
                    IItemLabelProvider.class );
            result.append( labelProvider.getText( v ) );
            first = false;
        }
